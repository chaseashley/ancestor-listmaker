...
export class AhnenTable extends React.Component {
  ...
  createRows(data, descendantJson, ancestorList, generations) {
    let rows = [];
  ...
  render() {
    return (
      <tbody>{this.createRows(this.props.tableData, this.props.descendantJson, this.props.ancestorList, this.props.generations)}</tbody>
    );
  }
