...
export class AhnenTable extends React.Component {
  ...
  createRows(data, descendantJson, ancestorList, generations, fullname, multiples, multiplesArray) {
    let rows = [];
    for (let i = 0; i < data.length; i++) {
      rows.push(<tr key={i}>
        ...
        &nbsp;<Link to={{ pathname: '/lines', endAncestor: data[i], descendantJson: descendantJson, ancestorList: ancestorList, generations: generations, fullname: fullname}}><img src={rfimage}/></Link>
        </td>
  ...
  render() {
    return (
      <tbody>{this.createRows(this.props.tableData, this.props.descendantJson, this.props.ancestorList, this.props.generations, this.props.fullname, this.props.multiples, this.props.multiplesArray)}</tbody>
    );
  }
